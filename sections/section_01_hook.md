...
**Finding 2: The long-lived directions are hidden inside the high-variance PCA span.** They do not sit in a quiet low-variance corner of the residual stream. They live at rotations inside the high-variance subspace that the principal components themselves do not occupy.

**Finding 3: The persistent state occupies a subspace, not the full ambient space.** The long-lived directions form a proper geometric subspace of the residual stream. This is different from working memory in our brains, where the relevant signals are distributed more broadly across cortex with no clean geometric boundary.
